#!/usr/bin/env python3
"""
Seed additional homeowners WITHOUT clearing existing ones
"""
import random
import uuid
...
def seed_additional_homeowners(num_homeowners=30, center_lat=37.7749, center_lng=-122.4194, radius_miles=25):
    """
    Add homeowner data to database around a specific location WITHOUT clearing existing data
    ...
    try:
        ...
        final_count = db.query(Homeowner).count()
        new_count = final_count - existing_count

        print(f"✅ Successfully added {new_count} homeowner applicants!")
        print(f"   Center: ({center_lat}, {center_lng})")
        print(f"   Radius: {radius_miles} miles")
        print(f"   Total homeowners now: {final_count}")
...
if __name__ == "__main__":
    ...
    print(f"Adding {num_homeowners} homeowners around ({center_lat}, {center_lng}) within {radius_miles} miles...")
    seed_additional_homeowners(num_homeowners, center_lat, center_lng, radius_miles)
